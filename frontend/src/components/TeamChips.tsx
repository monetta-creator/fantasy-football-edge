      ...
      {Object.entries(teams).map(([slot, name]) => {
        ...
        return (
          <button key={slot} type="button" disabled={busy} onClick={() => onPick(n)} className="text-left rounded-[8px] px-2.5 py-2 border text-[12px] leading-tight"
            ...
            <span className="block font-semibold" style={{ overflowWrap: "anywhere" }}>{me ? "Me" : name}</span>
            <span className="block text-[10px] opacity-70">{slot}{clock ? " · on the clock" : me ? ` · ${name}` : ""}</span>
          </button>
        );
      })}
    </div>
