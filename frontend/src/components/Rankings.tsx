...
const GRID = "grid grid-cols-[2rem_2.2rem_minmax(0,1fr)_3.4rem_3.4rem_3.4rem_4rem_8.6rem] gap-x-2";
  ...
  return (
    <div className="card p-3">
      ...
      <div className="max-h-[70vh] overflow-y-auto">
        <div className={`${GRID} text-[11px] muted px-1 py-1 border-b line sticky top-0 z-10`} style={{ background: "var(--card)" }}>
        ...
        {rows.map((p, i) => (
          <div key={p.id}>
            ...
            <div className={`${GRID} items-center text-[13px] py-1.5 px-1 border-b line tabular ${p.drafted ? "opacity-50" : ""}`}>
              ...
              <span className="flex gap-1 justify-end">{!p.drafted && <><button disabled={busy} className="pill text-[11px]" onClick={() => onGone(p)}>Gone</button><button disabled={busy} className="pill text-[11px]" style={{ borderColor: "var(--green)" }} onClick={() => onMine(p)}>Mine</button></>}</span>
            </div>
          </div>
        ))}
      </div>
