  ...
  return (
    <div className="space-y-4">
      ...
      <S title="Draft">
        ...
        <M name="Gone by #N">Probability the player is taken before your next pick, conditional on still being available now. Only decision-relevant when it is low (you could wait a round on him); shown in the Rankings list and inside the Decide reason list.</M>
        <M name="Cost of waiting">Projected points lost if you skip the position now and take the best one still there at your next pick, from the simulations (best available now minus expected best at your next pick). The headline tile on Decide cards.</M>
        ...
        <M name="Confidence">Margin over the runner-up relative to the simulation standard error: High if the margin is at least 6 and more than 2 standard errors; Medium if at least 2.5 and more than 1; otherwise Low. On Decide cards, option A is compared with B and every other option with A, using the same rule on that pair&apos;s standard errors.</M>
        <M name="Before your pick (look-ahead)">When Decide runs before the picks ahead of you are marked, the simulator also guesses those picks, and a candidate is sometimes gone before your turn. His roster value then counts only the simulated drafts in which he was still on the board, so it answers &quot;if he is there, is he the pick?&quot;; the card shows that percentage and the number of drafts used. Candidates on the board in fewer than 40 drafts are listed as not evaluated. Each option is its own &quot;if he is there&quot; world, so two look-ahead values are not conditioned on both players being available at once; on the clock every candidate is available, all drafts count, and the comparison is exact.</M>
        <M name="Bull / bear case">Two sentences at the top of each Decide card, built from the same facts as the reason list, up to four clauses each. For the pick: the simulation edge when confidence is Medium or High, highest VORP or best at the position, the higher of the two source projections, the position cliff to your next pick (flagged when it is the steepest), a market discount, the 2025 ceiling (best week, share of 25+ games) and steadiness, a clean injury flag, source agreement. Against it: injury status with return date, the lower source projection, projected games, whether he is likely to still be there at your next pick, a steeper cliff at another position, a market premium, an edge inside simulation noise or the gap to A, the 2025 floor (worst week, share of sub-8 games) and volatility, position depth (waiver level at least 60% of his projection), bye overlap. The language model never writes these.</M>
      ...
      <S title="Week">
        ...
        <M name="2025 weekly points chart">Last season&apos;s weekly results re-scored with this league&apos;s rules (nflverse stats). The dashed green line is the 2026 projected points per game; the dotted grey line is the 3-week rolling average. Type a name in the benchmark box to overlay another player.</M>
        <M name="Consistency">Startable % = weeks at or above a positional threshold (QB 18, RB/WR 12, TE 9, K 8, DEF 7). Boom = weeks of 25+, bust = weeks under 8. Trend compares second-half to first-half average (±10%).</M>
        <M name="2025 rates">Per-game opportunity and efficiency from nflverse: targets, catch %, yards per target, carries, yards per carry, touches, first downs. Target share and rush share = the player&apos;s share of his team&apos;s targets or carries (weekly average). Air-yards share = share of the team&apos;s intended air yards. WOPR = 1.5 × target share + 0.7 × air-yards share (weighted opportunity). aDOT = average depth of target. RACR = receiving yards per air yard. YAC = yards after catch per reception. EPA = expected points added: per game, per target, per carry, or per dropback; positive means the play gained expected points for the offense.</M>
      </S>

      <S title="Market data">
        <M name="Vegas consistency">Points implied by our player projections for each NFL team versus the Vegas implied total. Teams outside ±15% are flagged and their skill players scaled by the square root of the ratio, capped at ±15%.</M>
        <M name="Sportsbook props">The market&apos;s weekly projection per stat. A line is the median; the over/under prices give the lean; anytime-TD odds become expected touchdowns. Converted to league points for the components that have a prop and blended 50/50 into the projection. Market − model above +2 or below −2 is worth a look.</M>
        <M name="Correlated simulation">Each game&apos;s score is drawn from its line; players scale with their team&apos;s score plus their own noise. Correlations between your starters (and against your opponent&apos;s) come from these draws and explain why a stack raises your ceiling and lowers your floor.</M>
        <M name="Kalshi">Prediction-market prices: win probability per team, implied medians for totals, spreads and team totals, and player ladders where priced. Useful as a second opinion on the line; thin liquidity until close to kickoff.</M>
      </S>
